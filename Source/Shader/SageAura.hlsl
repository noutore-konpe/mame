...
Texture2D emissiveMap : register(t15);
...
float4 main(PSIn psIn) : SV_TARGET
{
    ...
    float3 finalColor = ambient + diffuse + specular + rim + foot;
    
    float3 emissive = emissiveMap.Sample(samplerStates[POINT], psIn.emissiveTexcoord).rgb;
    finalColor += emissive * emissiveIntensity * emissiveColor.rgb;
    
    return float4(finalColor, alpha) * float4(psIn.color.rgb * 0.45, psIn.color.a);
    //return float4(finalColor, alpha) * psIn.color*1/2;
}